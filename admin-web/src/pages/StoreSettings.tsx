...
import { Upload, ImageIcon, Loader2, Save, CheckCircle, XCircle } from 'lucide-react';
...
export default function StoreSettings() {
  ...
  const uploadOriginalImage = async (file: File) => {
    setUploading(true);
    try {
      const res = await fallbackOriginalUpload(file, { onProgress: (p) => setCompressionProgress(p.percent) });
      const url = res?.url || '';
      setStoreAvatar(url);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res: UploadResult = await smartUpload(file, { onProgress: (p) => setCompressionProgress(p.percent) });
      ...
      if (res.success && res.data) {
        ...
        setCompressionResult({ success: false, message: res.message || '图片压缩失败', allowOriginalUpload: true });
        showConfirm('压缩失败', `${res.message || '图片压缩失败'}\n\n图片大小：${formatFileSize(file.size)}\n\n是否继续上传原图？`, () => { uploadOriginalImage(file) }, () => { if (fileInputRef.current) fileInputRef.current.value = '' });
      }
    } catch (err: any) {
      setCompressionResult({ success: false, message: err?.message || '图片压缩请求失败', allowOriginalUpload: true });
      showConfirm('压缩失败', `${err?.message || '图片压缩请求失败'}\n\n图片大小：${formatFileSize(file.size)}\n\n是否继续上传原图？`, () => { uploadOriginalImage(file) }, () => { if (fileInputRef.current) fileInputRef.current.value = '' });
    } finally {
    ...
    <div className="max-w-2xl">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-[#94A3B8] mt-1">设置店铺名称和头像，将在小程序订单列表中显示</p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-full overflow-hidden bg-[#F1F5F9] border-2 border-[#E2E8F0] flex-shrink-0 relative">
                {previewUrl ? (
                  <>
                    <img className={imgBroken ? 'hidden' : 'w-full h-full object-cover'} src={previewUrl} onError={() => setImgBroken(true)} onLoad={() => setImgBroken(false)} />
                    {imgBroken && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-[#94A3B8]">
                        <ImageIcon size={24} /><span className="text-[11px] mt-1">暂无头像</span>
                      </div>
                    )}
                  </>
                ) : (
              ...
              <div>
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={compressing || uploading} className="flex items-center gap-1.5 px-4 py-2 border border-[#E2E8F0] rounded-lg text-sm text-[#334155] hover:bg-[#F8FAFC] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {compressing ? <Loader2 className="animate-spin" size={16} /> : uploading ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />}
                ...
                {compressing && (
                  <div>
                    <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-[#2563EB] rounded-full transition-all duration-300" style={{ width: `${compressionProgress}%` }} />
                    </div>
                ...
                {compressionResult && !compressing && (
                  <div className={`flex items-center gap-2 p-2 rounded-lg ${compressionResult.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    {compressionResult.success ? (
                      <>
                        <CheckCircle size={16} />
                        <span className="text-sm">压缩成功！压缩率 {compressionResult.data?.compressionRatio}%
                          {compressionResult.data && <span className="text-xs text-gray-500 ml-1">({formatFileSize(compressionResult.data.originalSize)} → {formatFileSize(compressionResult.data.compressedSize)})</span>}
                        </span>
                      </>
                    ) : (
                      <><XCircle size={16} /><span className="text-sm">{compressionResult.message}</span></>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="flex justify-end pt-4">
            <button onClick={handleSubmit} disabled={isSaving} className="flex items-center gap-1.5 px-6 py-2.5 bg-[#2563EB] text-white rounded-lg text-sm font-medium hover:bg-[#1D4ED8] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
